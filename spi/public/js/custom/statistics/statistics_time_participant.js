...
{
    var query = config_queries_participantStatistics;
...
function generateParticipantStatistics()
{
    executeQueryAction(config_queries_participantStatistics, createParticipantChart);
}
...
function createParticipantChart(data)
{
    ...
    $.each(data.results.bindings, function(i, row) {
        ...
        var userName = row['pName'].value.replace('@', '_').replace('.', '_');
    ...
    $.each(tmp, function(i, user) {
        ...
        $.each(workflows, function(i, workflow) {
            ...
            //drawParticipantChart(chartName, entries, workflopName);
...
function drawParticipantChart(element, entries, title)
{
    // prepare chart data
    var sampleData = [
