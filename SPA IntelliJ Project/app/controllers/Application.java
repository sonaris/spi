...
public class Application extends Controller {
    ...
    public static Result getQueryResult()
    {
        ...
        try
        {
            ...
            else if (query.contains("ASK"))
            {
                ...
                return ok("{\"answer\":\""+String.valueOf(result)+"\"}");
            }
            else return ok("{}");
        }
        catch (Exception e)
    ...
    public static Result saveQuery()
    {
        try {
            ...
            //Create new content
            Resource r = dataModel.createResource(baseURI + "#Query_" + name.replaceAll("\\s+", "")).addProperty(RDF.type, ontModel.getOntClass(Settings.queryOntologyNameSpace + "#Query"));
            ...
            ResIterator resources = dataModel.listResourcesWithProperty(ontModel.getProperty(Settings.systemOntologyNameSpace + "#hasAccountName"), session("account"));
            Resource account = resources.nextResource();

            r.addProperty(ontModel.getOntProperty(Settings.spaOntologyNameSpace + "#createdByAccount"), account);
    ...
    public static Result deleteQuery()
    {
        ...
        try
        {
            ...
            String action =
                    "DELETE { ?query ?property ?value } \n"
                    +"WHERE \n"
                    +"{ ?query ?property ?value. \n"
                    +   "FILTER (str(?query) = \""+uri+"\"). \n"
                    +"}";
    ...
    public static Result getQueries()
    {
        ...
        try
        {
            String result = DBLogic.getSavedQueries(session("account"));
    ...
    public static Result importRDF() {
        ...
        try {
            ...
            Http.MultipartFormData.FilePart file = body.getFile("rdf");
            ...
            if (file != null ) {
                ...
                //Insert RDF to database
                DBLogic.readRDF(fileStream,"DB_Analysis");
